...
/*
	pass multiple fish at a time. The buffers between the workers can hold one to 
	BATCH_SIZE fish. the entire batch must be finished before the batch can be put into a 
	buffer. And the entire batch is removed from the buffer.
*/


#define NUM_WORKERS  200
#define NUM_FISH 3000
#define BUFFER_SIZE 30 // ->batch size
...
void* worker_func(void* argz){
	...
	while(!finish){
		...
		if(info->id>0){ // if not the first worker, consume
			...
			buffer[info->id-1].count-=BUFFER_SIZE;// take batch of fish and do work
			for(int i=0;i<BUFFER_SIZE;i++){
				workee[i]=buffer[info->id-1].stock[buffer[info->id-1].count+i];
			}
			...
			pthread_mutex_unlock(&lock[info->id-1]);
 			...
 			if(remaining_fish>0){
 				remaining_fish-=BUFFER_SIZE;
 				for(int i=0;i<BUFFER_SIZE;i++){
					workee[i]=fish+fish_count+i;
					printf("fish  %d taken\n",remaining_fish+i);
					workee[i]->Tstart=get_time_sec();
				}
		...
		if(info->id < NUM_WORKERS-1){// if not the last worker, produce and put fish onto next buffer
			...
			// add to buffer
			for(int i=0;i<BUFFER_SIZE;i++){
				buffer[info->id].stock[buffer[info->id].count+i]=workee[i];// add fish object to buffer
			}
			
			buffer[info->id].count+=BUFFER_SIZE;
			...
			for(int i=0;i<BUFFER_SIZE;i++){
				workee[i]->Tend= get_time_sec();
				printf("Fish  %d is done \n",fish_count);
			}
		}
		
		// break statement
		fish_count+=BUFFER_SIZE;
