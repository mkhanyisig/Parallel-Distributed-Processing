...
__device__  __host__  float std(float *array, int N){
	float sum = array[0];
...
__device__  __host__  float printArray(float *array, int N){
	int i;
	printf("\n");
	for (i = 0; i < N; i++){
			printf(" %f ", array[i]);
...
__device__  __host__  void swapArrays(float **a, float **b){
    float *c=*a;
    *a=*b;
    *b=c;
}
...
__device__  __host__  float** add_arr(float **m1,float **m2,int row,int col){
...
__device__  __host__  float *createAscendingArray(int N) {
...
__device__  __host__  float dot_product(float v[], float u[], int n){
	int i;
	float result=0.0;
	for (i = 0; i < n; i++){
		result += v[i]*u[i];
...
__device__  __host__  float randFloat() {
